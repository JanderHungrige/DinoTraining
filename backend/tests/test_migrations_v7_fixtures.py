"""A database at the v6 shape: everything v5 had, plus the `imported` provenance.
...
CREATE TABLE boxes (
    ...
    provenance TEXT NOT NULL CHECK (provenance IN ({_V6_PROVENANCE})),
...
def at_v6() -> sqlite3.Connection:
    """A v6 database with one dataset, one image, one box and one mask already in it."""
    connection = _connect(V6_SCHEMA)
    connection.execute(
