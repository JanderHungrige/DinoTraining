...
def build_class_vocabulary(
    ...
    """Distinct classes across all positive annotations, **sorted**.
    ...
    Spans boxes *and* masks. A dataset generated by Grounded SAM stores its classes on the
    masks, so a box-only vocabulary would come back empty and the run would refuse with
    "no classes" on a dataset visibly full of them.
    """
    names = {
        _class_name(box)
    ...
    names |= {
        _class_name(mask)
        for _, _, _, _, image_masks in (masks or [])
...
def classes_for_task(sample_set: SampleSet, task: str) -> tuple[str, ...]:
    ...
    **Segmentation gets a background class and nothing else does.** Every pixel of a
    segmentation belongs to some class, and most of them belong to none of the annotated
    ones; without an explicit background the loss can only ignore those pixels, and a model
    that never sees background learns to label the entire frame. Detection has no such
    problem — a cell with no box is simply not a positive.

    Index 0, deliberately, and it is worth knowing why beyond convention: the overlay
    registry already treats `class_names[0] === 'background'` as the signal to draw class 0
    ...
    if task == "segmentation":
        return (BACKGROUND_CLASS, *sample_set.class_names)
...
def build_samples(
    ...
    class_names = build_class_vocabulary(annotations, stored_masks)
    ...
    for image_id, path, width, height, boxes in annotations:
        ...
        for mask in image_masks:
            target = MaskTarget(
                class_index=index_of.get(_class_name(mask), 0),
                size=mask.rle.size,
