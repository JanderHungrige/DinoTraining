...
- **Backbone feature caching across heads.** One forward pass feeding N heads is the point
  of `multi-head-compose`; the open question is where the cache lives and when it is
  invalidated (backbone change, input change, preprocessing change). Note that preprocessing
  is *per head type* — `center-crop` for classification, `aspect-preserve` for dense tasks
  (doc 10) — so heads with different `geometry` cannot share a forward pass. That constraint
  shapes the cache key and may cap how much sharing is actually available.
