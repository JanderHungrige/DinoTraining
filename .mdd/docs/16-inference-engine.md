...
security_read_sites: []
known_issues:
  - "**Fixed 2026-08-20 (doc 31).** `detection_decode` performed no NMS at all, while the `dense-detector` head type has advertised \"NMS at inference\" since Wave 2 — the phrase existed only in the description string. Every patch cell covering an object emitted its own box, so one thermal person came back as 32 overlapping proposals. Duplicates count as false positives, so this depressed `map` as much as it cluttered review: adding class-aware NMS moved thermal 0.263 -> 0.404, blood 0.207 -> 0.387, chess 0.425 -> 0.525 on unchanged data and unchanged training. Found by running a trained detector on real images; no unit test could have caught it, because the fixture asserted one entry per cell — it encoded the bug as the contract."
