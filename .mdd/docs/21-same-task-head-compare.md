...
security_read_sites: []
known_issues:
  - "**Fixed 2026-08-20 (found in Wave 7.5).** A result outlived the image it described: `useHeadRun` cleared `result` on selection changes but not on *navigation*, so running a model on image 1 and paging through a folder drew image 1's boxes over every later image. Present since this feature shipped and invisible until someone ran a model and then paged — Jan hit it with RF-DETR, but trained heads were equally affected. Fixed by making the result **belong to** the image it was computed from: `useHeadRun` now takes the current path and derives visibility rather than clearing on navigation, which also restores an earlier image's own result when you page back, and drops a response that lands after the user has moved on. Four regression tests, each confirmed failing against the old code."
