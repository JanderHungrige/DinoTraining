...
known_issues:
  ...
  - "Community imports inherit their head *type's* preprocessing geometry, and nothing validates that the head was actually trained that way. `geometry` is a property of the HeadTypeSpec, not of the instance, so a community classifier trained at 336x336 gets assigned `center-crop @ 224` and runs quietly at the wrong resolution. The import path checks `embed_dim` (via `load_state_dict(strict=True)`) and backbone family (via `check_compatibility`), but never asks what input size the head expects — and because every head here is shape-agnostic (1x1 convs over any grid; a Linear over a fixed-width CLS), a resolution mismatch raises nothing and only shows up as degraded accuracy. The wave doc lists a required manifest of `backbone version, patch size, embed dim and task`; **input geometry belongs in that list and is missing**. Low severity while no real third-party DINOv2 head has been identified, which is still open."
